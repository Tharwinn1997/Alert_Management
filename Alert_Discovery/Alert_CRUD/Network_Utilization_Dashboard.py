...
app_c = Blueprint('app_c', __name__)


@app_c.route("/create_alert", methods=['POST'])
def create_alert():
    try:
        ...
        if metric == 'Inspeed':
            metric_name = ("SELECT\n  ROUND(8 * non_negative_derivative(mean(\"ifHCInOctets\"), 1m) / 60) as "
                           "\"inspeed\"\nFROM\n"
                           " \"ciscoInterface\"\nWHERE\n  sysName =~ /ACE-INTERNET-RTR\\.acedesigners\\.co\\.in$/ AND\n  "
                           "ifDescr='GigabitEthernet0/0/1' AND\n  $timeFilter\nGROUP BY\n  time(1m) fill(null);")
            panel_title = "Bandwidth Utilization for $host:$index"

        elif metric == 'Outspeed':
            metric_name = ("SELECT\n  ROUND(8 * non_negative_derivative(mean(\"ifHCOutOctets\"),1m) / 60) as "
                           "\"outspeed\"\nFROM\n  \"ciscoInterface\"\nWHERE\n  sysName =~ "
                           "/ACE-INTERNET-RTR\\.acedesigners\\.co\\.in$/ AND\n  ifDescr='GigabitEthernet0/0/1' AND\n  "
                           "$timeFilter\nGROUP BY\n  time(1m) fill(null);")
            panel_title = "Bandwidth Utilization for $host:$index"
                ...
                "data": [
                    {
                        ...
                        "model": {
                            "adhocFilters": [],
                            "alias": "GigabitEthernet0/0/1: $col",
                            "datasource": {
                                "type": "influxdb",
...
@app_c.route("/update_alert", methods=['PUT'])
def update_alert():
    try:
        data = request.get_json()
        ...
        if panel_title == "Bandwidth Utilization for $host:$index":
            metric_name = ("SELECT\n  ROUND(8 * non_negative_derivative(mean(\"ifHCInOctets\"), 1m) / 60) as "
                           "\"inspeed\"\nFROM\n"
                           " \"ciscoInterface\"\nWHERE\n  sysName =~ /ACE-INTERNET-RTR\\.acedesigners\\.co\\.in$/ AND\n  "
                           "ifDescr='GigabitEthernet0/0/1' AND\n  $timeFilter\nGROUP BY\n  time(1m) fill(null);")

        elif panel_title == "Bandwidth Utilization for $host:$index":
            metric_name = ("SELECT\n  ROUND(8 * non_negative_derivative(mean(\"ifHCOutOctets\"),1m) / 60) as "
                           "\"outspeed\"\nFROM\n  \"ciscoInterface\"\nWHERE\n  sysName =~ "
                           "/ACE-INTERNET-RTR\\.acedesigners\\.co\\.in$/ AND\n  ifDescr='GigabitEthernet0/0/1' AND\n  "
                           "$timeFilter\nGROUP BY\n  time(1m) fill(null);")
